...
// ------------------------------------------------------------------------------------------------------
function ToXml(str,js){
if(typeof(str)!="string" || str.search(/[\'\&\"\<\>\n]/)<0) return str;
return str.replace(/\&/g,"&amp;").replace(/\"/g,js?"\\&quot;":"&quot;").replace(/\'/g,"&apos;").replace(/\</g,"&lt;");
}
// ------------------------------------------------------------------------------------------------------
...
if(!R[0] || !C[0] || !D[0]) { GetElem("Grid2").style.display = "none"; return; } // Error - incomplete setting
...
if(!C0.length) { GetElem("Grid2").style.display = "none"; return } // Error - no values 
GetElem("Grid2").style.display = "";
...
for(var i=0;i<C0.length;i++) L += " C"+i+"='"+ToXml(C0[i],0)+"'";
L += "/>";
...
for(var j=0;j<R0.length;j++){
   ...
   for(var i=0;i<C0.length;i++){
      L += " C"+i+"Formula='GSum(\""+R[0]+"\",\""+ToXml(R0[j],1)+"\",\""+C[0]+"\",\""+ToXml(C0[i],1)+"\",\""+D[0]+"\")'";
      }
   L += TF+"/>";
   }
