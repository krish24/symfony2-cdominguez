    ...
    init:function(){
        ...
        if(this._build()){
            ...
            this.data={
                ...
                centerX:f.width/2,
                centerY:f.height/2,
                radius:d(".da-circular-progress",this.$elem).width()/2
                };
...
_getVal:function(f){
    if(this.config.percent){
        ...
        return f>1?this.data.endValue:Math.ceil(f*(this.data.endValue-this.data.startValue))
        }
    },
_update:function(h,k){
    ...
    if(k){
        ...
        this.context.fill();
        this.valueEl.innerHTML=this.config.percent?("<span>"+i+"%</span>"):("<span>"+i+"</span>/"+this.config.maxValue)
        }
    }
};
